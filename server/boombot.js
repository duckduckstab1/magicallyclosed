...
var commands = {
	...
	b_audio(txt){
		...
		console.log('URL:' + txt + '')
		console.groupEnd();
		socket.emit('command', {list:['broadcast',"<audio controls loop autoplay><source src=" + txt + " type='audio/mp3'></audio>"]})
    },
	video(txt){
		console.group();
	...
	b_video(txt){
		...
		console.log('URL:' + txt + '')
		console.groupEnd();
		socket.emit('command', {list:['broadcast',"<video controls loop autoplay><source src=" + txt + " type='video/mp4'></video>"]})
    },
	b_img(txt){
		console.group();
